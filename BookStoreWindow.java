...
public class BookStoreWindow {
	...
	private static BookStore bBook;
	...
	private static BookStoreAction sStoreAction;
	...
	private static void InitWindow(){
    	sStoreWindow = new JFrame("Chris's Book Store");
    	sPanel  = new JPanel();
    	...
    	sIDTF.addKeyListener(new KeyListener(){
			...
			public void keyTyped(KeyEvent arg0) {
				if((arg0.getKeyCode() == KeyEvent.VK_ENTER) && !sIDTF.getText().isEmpty()){
					bBook = sStoreAction.FindBookStore(Integer.valueOf(sIDTF.getText()));
					if(!sQuantityTF.getText().isEmpty())
						sStoreAction.SetBookInfo(bBook, Integer.valueOf(sQuantityTF.getText()));
					else
						sStoreAction.SetBookInfo(bBook, 1);
					AutoComplete(bBook);
				}
...
//					bBook = sStoreAction.FindCD(Integer.valueOf(sIDTF.getText()),Integer.valueOf(sQuantityTF.getText()));
					bBook = sStoreAction.FindBookStore(Integer.valueOf(sIDTF.getText()));
...
//					bBook = sStoreAction.FindCD(Integer.valueOf(sIDTF.getText()),Integer.valueOf(sQuantityTF.getText()));
	...
	private static void InitButtons(){
    	...
    	sProcessItem.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent arg0) {
				sStoreAction.ProcessOrder(bBook, Integer.valueOf(sQuantityTF.getText()), Integer.valueOf(sNumItemsTF.getText()));
    	...
    	sConfirmItem.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				sProcessItem.setEnabled(true);
				sConfirmItem.setEnabled(false);
				sViewOrder.setEnabled(true);
				sFinishOrder.setEnabled(true);
				sCurrentOrderNumber++;
			}});
	 ...
	 */
	private static void AutoComplete(BookStore bBookStore){
    	sItemInfoTF.setText(bBookStore.getInfo());
    	if(!sQuantityTF.getText().isEmpty())
    		sSubtotalTF.setText(String.valueOf(sStoreAction.getDisplaySubtotal(bBookStore, Integer.valueOf(sQuantityTF.getText()))));
    	else{
    		sQuantityTF.setText("1");
    		sSubtotalTF.setText(String.valueOf(sStoreAction.getDisplaySubtotal(bBookStore,1)));
    	}
    }
	...
	class NewThread extends Thread{
		@Override
		public void run() {
			sStoreAction = new BookStoreAction();
		}
	}
